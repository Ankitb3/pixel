...
export function EditorTopBar({ projects }) {
  ...
  const handleUndo = async () => {
    ...
    try {
      ...
      if (previousState) {
        ...
        toast.success("Undo last action");
      }
    } catch (error) {
  ...
  const handleRedo = async () => {
    ...
    try {
      ...
      if (nextState) {
        ...
        toast.success("Redo last action");
      }
    } catch (error) {
  ...
  const handleExport = async (exportConfig) => {
    ...
    try {
      ...
      canvasEditor.setDimensions({
        width: projects.width * currentZoom,
        height: projects.height * currentZoom,
      });
      canvasEditor.requestRenderAll();

      // Download the image
      const link = document.createElement("a");
      link.download = `${projects.title}.${exportConfig.extension}`;
      link.href = dataURL;
  ...
  const handleResetToOriginal = async () => {
    if (!canvasEditor || !projects || !projects.originalImageUrl) {
    ...
    try {
      ...
      // Load original image
      const fabricImage = await FabricImage.fromURL(projects.originalImageUrl, {
        crossOrigin: "anonymous",
      });

      // Calculate proper scaling
      const imgAspectRatio = fabricImage.width / fabricImage.height;
      const canvasAspectRatio = projects.width / projects.height;
      const scale =
        imgAspectRatio > canvasAspectRatio
          ? projects.width / fabricImage.width
          : projects.height / fabricImage.height;

      fabricImage.set({
        left: projects.width / 2,
        top: projects.height / 2,
        originX: "center",
        originY: "center",
        scaleX: scale,
        scaleY: scale,
      ...
      await updateProject({
        projectId: projects._id,
        canvasState: canvasJSON,
        currentImageUrl: projects.originalImageUrl,
    ...
    <>
      <div className="border-b px-6 py-3">
        ...
        <div className="flex items-center justify-between mb-4">
          {/* Left: Back button and projects name */}
          ...
          <h1 className="font-extrabold capitalize">{projects.title}</h1>
          ...
          <div className="flex items-center gap-3">
            ...
            <Button
              ...
              disabled={isSaving || !projects.originalImageUrl}
              className="gap-2"
            >
              {isSaving ? (
                <>
            ...
            <DropdownMenu>
              ...
              >
                <div className="px-3 py-2 text-sm text-white/70">
                  Export Resolution: {projects.width} × {projects.height}px
                </div>
